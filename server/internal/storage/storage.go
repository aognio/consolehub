...
// ListHostsByTenant returns all hosts associated with a specific tenant or unassigned.
func (s *Storage) ListHostsByTenant(tenantID string) ([]models.Host, error) {
	if tenantID == "" {
		return s.ListHosts()
	}
	assocRecs, err := s.App.FindRecordsByFilter("ch_host_tenants", "tenant_id = {:tenant_id}", "", 500, 0, dbx.Params{"tenant_id": tenantID})
	if err != nil {
		return nil, err
	}
	...
	for _, assoc := range assocRecs {
		hostIDSet[assoc.GetString("host_id")] = true
	}

	allHosts, err := s.ListHosts()
	if err != nil {
		return nil, err
	}

	matchedHosts := make([]models.Host, 0, len(allHosts))
	for _, host := range allHosts {
		if hostIDSet[host.ID] {
			matchedHosts = append(matchedHosts, host)
		} else {
			tIDs, _ := s.GetHostTenantIDs(host.ID)
			if len(tIDs) == 0 {
				matchedHosts = append(matchedHosts, host)
			}
		}
...
// ListTenantsByHost returns all tenants associated with a host.
func (s *Storage) ListTenantsByHost(hostID string) ([]models.Tenant, error) {
	assocRecs, err := s.App.FindRecordsByFilter("ch_host_tenants", "host_id = {:host_id}", "", 500, 0, dbx.Params{"host_id": hostID})
	if err != nil {
		return nil, err
	}

	tenants := make([]models.Tenant, 0, len(assocRecs))
	for _, assoc := range assocRecs {
		tenantID := assoc.GetString("tenant_id")
		tRec, err := s.App.FindRecordById("ch_tenants", tenantID)
		if err == nil {
			tenants = append(tenants, models.Tenant{
				ID:        tRec.Id,
				Name:      tRec.GetString("name"),
				Slug:      tRec.GetString("slug"),
				Active:    tRec.GetBool("active"),
				CreatedAt: tRec.GetDateTime("created").Time(),
				UpdatedAt: tRec.GetDateTime("updated").Time(),
			})
		}
	}
	return tenants, nil
...
// SyncHostTenants updates all tenant associations for a host.
func (s *Storage) SyncHostTenants(hostID string, tenantIDs []string) error {
	existingIDs, err := s.GetHostTenantIDs(hostID)
	if err != nil {
		existingIDs = []string{}
	}
	...
	for tID := range newMap {
		if !existingMap[tID] {
			_ = s.AssociateHostTenant(hostID, tID)
		}
	}

	for tID := range existingMap {
		if !newMap[tID] {
			_ = s.DissociateHostTenant(hostID, tID)
		}
	}
