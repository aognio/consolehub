...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/hosts":
		...
		hosts, _ := h.services.ListHostsByTenant(r.Context(), activeTenantID)
		tenants, _ := h.services.ListTenants(r.Context())

		type HostWithTenants struct {
			*models.Host
			Tenants []*models.Tenant
		}

		hostVMs := make([]HostWithTenants, 0, len(hosts))
		for _, host := range hosts {
			tList, _ := h.services.ListTenantsByHost(r.Context(), host.ID)
			hostVMs = append(hostVMs, HostWithTenants{
				Host:    host,
				Tenants: tList,
			})
		}
...
func (h *Handler) handleHostPost(w http.ResponseWriter, r *http.Request) {
	...
	case "create":
		...
		_ = r.ParseForm()
		tenantIDs := r.Form["tenant_ids"]
		activeTenantID := h.getActiveTenantID(r)
		if len(tenantIDs) == 0 && activeTenantID != "" {
			tenantIDs = []string{activeTenantID}
		}

		if slug != "" && hostname != "" {
			host, err := h.services.RegisterHost(r.Context(), slug, hostname, fqdn, displayName, platform)
			if err != nil {
				log.Printf("[UI] RegisterHost error: %v", err)
			} else if host != nil {
				if len(tenantIDs) > 0 {
					for _, tID := range tenantIDs {
						if tID != "" {
							_ = h.services.AssociateHostTenant(r.Context(), host.ID, tID)
						}
					}
				} else {
					allTenants, _ := h.services.ListTenants(r.Context())
					for _, t := range allTenants {
						_ = h.services.AssociateHostTenant(r.Context(), host.ID, t.ID)
					}
				}
			}
		}
	...
	case "sync_tenants":
		...
		if hostID != "" {
			_ = r.ParseForm()
			tenantIDs := r.Form["tenant_ids"]
			_ = h.services.SyncHostTenants(r.Context(), hostID, tenantIDs)
		}
	}
